...
為在上下文壓力或品質下降前完成換班，讓角色長期保留、Codex 窗口分代，
並由 LATTICE／PostgreSQL 保存工作歸屬、交接狀態與驗收證據；窗口、上下文、
排程與封存仍使用 Codex 原生能力。新窗口須先核對工程與未完成事項，驗收通過
才切換唯一負責者並封存舊窗口。

2026-09-07 本機第一版已在四個登記角色啟用，以 20 個可核對的完成回合為門檻，
並完成進度角色一輪真實換班與新入口巡檢。本次公開更新只有說明，實裝程式尚未
由此文件 PR 發布或合併；其他主機、雲端及所有直接檔案操作不在已驗證覆蓋範圍。
完整分工、接入方式與驗收邊界見 [Bot 生命週期與交接](docs/bot-lifecycle.md)。
