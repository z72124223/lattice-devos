-- TASK-079: exact schema-v5 to schema-v6 foreman coordination binding.
...
ALTER TABLE control.project_registry_commands
    ADD CONSTRAINT project_registry_commands_persistence_profile CHECK (
        persistence_schema_version BETWEEN 4 AND 6
...
BEGIN
    ...
    IF p_global_schema_version IS NULL
       ...
       OR v_manifest_entry_count IS DISTINCT FROM 7
       ...
       OR (SELECT pg_catalog.count(*)
            ...
            WHERE c.singleton = true
              ...
              AND c.min_writer = 6 AND c.max_writer = 6
    ...
    IF NOT v_compatibility_found
       OR v_schema_version IS DISTINCT FROM 6
       OR v_min_reader IS DISTINCT FROM 6
       OR v_max_reader IS DISTINCT FROM 6
       OR v_min_writer IS DISTINCT FROM 6
       OR v_max_writer IS DISTINCT FROM 6
...
BEGIN
    ...
    IF v_manifest_entry_count IS DISTINCT FROM 7 OR (
        SELECT pg_catalog.count(*)
          FROM ONLY control.schema_compatibility AS c
         WHERE c.singleton = true
           AND c.current_schema_version = 6
...
BEGIN
    ...
    IF NOT v_compatibility_found
       OR v_global_schema_version IS DISTINCT FROM 6
       OR v_min_reader IS DISTINCT FROM 6
       OR v_max_reader IS DISTINCT FROM 6
       OR v_min_writer IS DISTINCT FROM 6
       OR v_max_writer IS DISTINCT FROM 6
...
BEGIN
    ...
    IF session_user <> 'lattice_runtime_login'
       ...
       OR p_persistence_schema_version IS DISTINCT FROM 6
...
BEGIN
    ...
    SELECT c.xmin = pg_catalog.pg_current_xact_id()::xid
     ...
     WHERE c.ordinal = p_ordinal AND c.command_id = p_command_id
       ...
       AND c.persistence_schema_version = 6
...
COMMENT ON SCHEMA memory IS 'LATTICE_DEVOS_MEMORY_SCHEMA_V6';
COMMENT ON SCHEMA readmodel IS 'LATTICE_DEVOS_READMODEL_SCHEMA_V6';
...
CREATE FUNCTION control.task_ledger_read_foreman_snapshots_v1(p_stream_id bytea)
RETURNS TABLE (
