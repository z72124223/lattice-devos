-- Store V8 runtime successor. Append-only reconciliation for the immutable
-- external-adoption prefix; Foreman remains owned by its extension setup.
...
ALTER TABLE control.project_registry_commands
    ADD CONSTRAINT project_registry_commands_persistence_profile CHECK (
        persistence_schema_version BETWEEN 4 AND 8
...
BEGIN
    ...
    IF NOT v_compatibility_found
       OR v_schema_version IS DISTINCT FROM 8
       OR v_min_reader IS DISTINCT FROM 8
       OR v_max_reader IS DISTINCT FROM 8
       OR v_min_writer IS DISTINCT FROM 8
       OR v_max_writer IS DISTINCT FROM 8
...
BEGIN
    ...
    IF v_manifest_entry_count IS DISTINCT FROM 10 OR (
        SELECT pg_catalog.count(*)
          FROM ONLY control.schema_compatibility AS c
         WHERE c.singleton = true
           AND c.current_schema_version = 8
...
BEGIN
    ...
    IF NOT v_compatibility_found
       OR v_global_schema_version IS DISTINCT FROM 8
       OR v_min_reader IS DISTINCT FROM 8
       OR v_max_reader IS DISTINCT FROM 8
       OR v_min_writer IS DISTINCT FROM 8
       OR v_max_writer IS DISTINCT FROM 8
...
CREATE OR REPLACE FUNCTION control.task_ledger_finalize_general_intake_v1(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text,
    p_stream_id bytea,
    p_project_id text,
...
BEGIN
    ...
    IF session_user <> 'lattice_runtime_login'
       ...
       OR p_persistence_schema_version IS DISTINCT FROM 8
...
BEGIN
    ...
    SELECT c.xmin = pg_catalog.pg_current_xact_id()::xid
     ...
     WHERE c.ordinal = p_ordinal AND c.command_id = p_command_id
       ...
       AND c.persistence_schema_version = 8
