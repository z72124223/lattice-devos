-- TASK-075: exact schema-v4 to schema-v5 forward profile.
...
ALTER TABLE control.task_ledger_commands
    ADD CONSTRAINT task_ledger_commands_closed_values CHECK (
        event_kind IN ('TASK_CREATED', 'AUTONOMY_RECEIPT_RECORDED', 'STATE_TRANSITION',
...
ALTER TABLE control.project_registry_commands
    ADD COLUMN persistence_schema_version smallint NOT NULL DEFAULT 4;
ALTER TABLE control.project_registry_commands
    ADD COLUMN persistence_manifest_sha256 text NOT NULL DEFAULT 'df3f7ca3687afaa0d1f676158725e6d2f06670e0612df7482aa9d4d244b59f0f';
ALTER TABLE control.project_registry_commands
    ALTER COLUMN persistence_schema_version DROP DEFAULT,
    ALTER COLUMN persistence_manifest_sha256 DROP DEFAULT,
    ADD CONSTRAINT project_registry_commands_persistence_profile CHECK (
        persistence_schema_version BETWEEN 4 AND 5
        AND persistence_manifest_sha256 ~ '^[0-9a-f]{64}$'
        AND persistence_manifest_sha256 <> pg_catalog.repeat('0', 64)
    );

CREATE FUNCTION control.store_prepare_v5(
...
BEGIN
    ...
    IF p_global_schema_version IS NULL
       ...
       OR v_manifest_entry_count IS DISTINCT FROM 6
       ...
       OR (SELECT pg_catalog.count(*)
            ...
            WHERE c.singleton = true
              ...
              AND c.min_reader = 5 AND c.max_reader = 5
              AND c.min_writer = 5 AND c.max_writer = 5
    ...
    IF NOT v_compatibility_found
       OR v_schema_version IS DISTINCT FROM 5
       OR v_min_reader IS DISTINCT FROM 5
       OR v_max_reader IS DISTINCT FROM 5
       OR v_min_writer IS DISTINCT FROM 5
       OR v_max_writer IS DISTINCT FROM 5
...
CREATE FUNCTION control.store_finalize_v5(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text,
    p_store_contract_version smallint,
    p_transaction_id text,
...
CREATE FUNCTION control.store_current_head_v5(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text,
    p_project_id text,
    p_project_snapshot_id text,
...
CREATE FUNCTION control.task_ledger_prepare_v3(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text,
    p_stream_id bytea,
    p_command_id text
...
BEGIN
    ...
    IF v_manifest_entry_count IS DISTINCT FROM 6 OR (
        SELECT pg_catalog.count(*)
          FROM ONLY control.schema_compatibility AS c
         WHERE c.singleton = true
           AND c.current_schema_version = 5
...
CREATE FUNCTION control.task_ledger_read_head_v3(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text,
    p_stream_id bytea,
...
BEGIN
    ...
    IF NOT v_compatibility_found
       OR v_global_schema_version IS DISTINCT FROM 5
       OR v_min_reader IS DISTINCT FROM 5
       OR v_max_reader IS DISTINCT FROM 5
       OR v_min_writer IS DISTINCT FROM 5
       OR v_max_writer IS DISTINCT FROM 5
...
CREATE FUNCTION control.task_ledger_read_events_v3(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text,
    p_stream_id bytea
)
...
CREATE FUNCTION control.task_ledger_read_commands_v3(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text,
    p_stream_id bytea
)
...
CREATE FUNCTION control.task_ledger_finalize_v3(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text,
    p_stream_id bytea,
    p_project_id text,
...
CREATE FUNCTION control.project_registry_prepare_v2(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text,
    p_command_id text,
    p_request_digest bytea,
...
CREATE FUNCTION control.project_registry_read_state_v2(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text
)
...
CREATE FUNCTION control.project_registry_read_observations_v2(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text
)
...
CREATE FUNCTION control.project_registry_read_projects_v2(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text
)
...
CREATE FUNCTION control.project_registry_read_commands_v2(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text
)
...
CREATE FUNCTION control.project_registry_read_reservations_v2(
    p_global_schema_version smallint,
    p_global_manifest_sha256 text
)
...
CREATE FUNCTION control.project_registry_stage_command_v2(
...
BEGIN
    ...
    IF session_user <> 'lattice_runtime_login'
       ...
       OR p_persistence_schema_version IS DISTINCT FROM 5
...
CREATE FUNCTION control.project_registry_stage_project_v2(
    p_global_schema_version smallint, p_global_manifest_sha256 text,
...
CREATE FUNCTION control.project_registry_finalize_v2(
    p_global_schema_version smallint, p_global_manifest_sha256 text,
...
BEGIN
    ...
    SELECT c.xmin = pg_catalog.pg_current_xact_id()::xid
     ...
     WHERE c.ordinal = p_ordinal AND c.command_id = p_command_id
       ...
       AND c.persistence_schema_version = 5
...
GRANT EXECUTE ON FUNCTION control.task_ledger_read_head_v3(smallint, text, bytea, text, text) TO lattice_runtime;
...
COMMENT ON SCHEMA control IS 'LATTICE_DEVOS_CONTROL_SCHEMA_V5';
COMMENT ON SCHEMA memory IS 'LATTICE_DEVOS_MEMORY_SCHEMA_V5';
COMMENT ON SCHEMA readmodel IS 'LATTICE_DEVOS_READMODEL_SCHEMA_V5';
