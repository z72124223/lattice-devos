-- LATTICE PostgreSQL Writer Lease append-only successor profile v4.
-- Writer v3 remains immutable for retained Store-v6 databases. This successor
-- stages exact Store-v7 runtime bindings without relabelling the v3 identity.

ALTER TABLE writer_lease.writer_lease_extension_identity
    DROP CONSTRAINT writer_lease_extension_identity_profile_v3,
    ADD CONSTRAINT writer_lease_extension_identity_profile_v4 CHECK (
...
ALTER TABLE writer_lease.writer_lease_extension_ledger
    ...
    ADD CONSTRAINT writer_lease_extension_ledger_profile_v4 CHECK (
        extension_id = 'lattice-writer-lease'
        AND (
...
CREATE FUNCTION writer_lease.writer_lease_bind_runtime_v4(
...
AS $lattice_writer_lease_bind_runtime_v4$
DECLARE
    v_count bigint;
...
CREATE FUNCTION writer_lease.writer_lease_load_for_update_v4(
    p_project_id text,
...
SET lock_timeout = '5s'
SET statement_timeout = '30s'
AS $lattice_writer_lease_load_for_update_v4$
DECLARE
...
COMMENT ON SCHEMA writer_lease IS 'LATTICE_WRITER_LEASE_SCHEMA_V4';
...
COMMENT ON FUNCTION writer_lease.writer_lease_bind_runtime_v4(
    text,bigint,bytea,text,text,text,text,text
) IS 'PHASE3_GLOBAL_SCHEMA_V7_GENERAL_TASK_INTAKE';
