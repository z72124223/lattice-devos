-- Writer-owned fixed runtime compatibility successor for exact Store V7/V8.
-- Writer identity, ledger, authority rows, and business semantics stay immutable.

CREATE OR REPLACE FUNCTION writer_lease.writer_lease_bind_runtime_v5(
...
CREATE OR REPLACE FUNCTION writer_lease.writer_lease_load_for_update_v5(
    p_project_id text,
     ...
     WHERE w.singleton AND w.extension_id = 'lattice-writer-lease'
       ...
       AND (
            (c.current_schema_version = 7
             AND c.min_reader = 7 AND c.max_reader = 7
             AND c.min_writer = 7 AND c.max_writer = 7
             AND c.manifest_sha256 = w.global_manifest_sha256)
            OR
            (c.current_schema_version = 8
             AND c.min_reader = 8 AND c.max_reader = 8
             AND c.min_writer = 8 AND c.max_writer = 8
