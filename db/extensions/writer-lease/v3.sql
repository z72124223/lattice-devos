-- LATTICE PostgreSQL Writer Lease append-only successor profile v3.
-- TASK-087 owns compatibility only. The future Store migration owns
-- FOREMAN_COORDINATION / FOREMAN_SNAPSHOT_RECORDED physical persistence.

ALTER TABLE writer_lease.writer_lease_extension_identity
    DROP CONSTRAINT writer_lease_extension_identity_profile,
    ADD CONSTRAINT writer_lease_extension_identity_profile_v3 CHECK (
...
ALTER TABLE writer_lease.writer_lease_extension_ledger
    DROP CONSTRAINT writer_lease_extension_ledger_profile,
...
CREATE FUNCTION writer_lease.writer_lease_bind_runtime_v3(
...
AS $lattice_writer_lease_bind_runtime_v3$
DECLARE
    v_count bigint;
       ...
       AND EXISTS (
           SELECT 1 FROM ONLY writer_lease.writer_lease_extension_ledger AS l
            WHERE l.singleton = w.singleton
              ...
              AND l.event_kind IN ('INSTALLED', 'REBOUND')
       );
...
CREATE FUNCTION writer_lease.writer_lease_load_for_update_v3(
    p_project_id text,
...
SET lock_timeout = '5s'
SET statement_timeout = '30s'
AS $lattice_writer_lease_load_for_update_v3$
DECLARE
...
BEGIN
    ...
    SELECT pg_catalog.count(*) INTO v_identity_count
     ...
     WHERE w.singleton AND w.extension_id = 'lattice-writer-lease'
       AND w.extension_schema_version = 3
       AND w.extension_path = 'db/extensions/writer-lease/v3.sql'
       AND w.global_schema_version = 6 AND w.required_memory_schema_version = 3
...
COMMENT ON SCHEMA writer_lease IS 'LATTICE_WRITER_LEASE_SCHEMA_V3';
...
COMMENT ON FUNCTION writer_lease.writer_lease_bind_runtime_v3(
    text,bigint,bytea,text,text,text,text,text
) IS 'TASK087_GLOBAL_SCHEMA_V6_FOREMAN_COORDINATION_FOREMAN_SNAPSHOT_RECORDED';
