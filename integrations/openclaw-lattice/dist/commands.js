...
export function parseLatticeArguments(input) {
    if (Buffer.byteLength(input, "utf8") > MAX_ARGUMENT_BYTES ||
...
function parseTaskTarget(tokens, offset) {
...
function validRevision(value) {
    return /^[1-9][0-9]*$/u.test(value) && BigInt(value) <= 18446744073709551615n;
}
function validDigest(value) {
    return DIGEST.test(value) && !/^0+$/u.test(value);
}
export function deriveCommandIdentities(sessionKey, command) {
...
export function canonicalArguments(command) {
...
function deriveIdentity(domain, sessionKey, canonical) {
