 ...
 */
export async function exchangeAuthenticatedFrames(options) {
    ...
    try {
        ...
        await writeAll(socket, encodeAuthenticatedPacket(REQUEST_MAGIC, epoch, helloNonce, options.helloPayload, sessionKey));
        await writeAll(socket, encodeAuthenticatedPacket(REQUEST_MAGIC, epoch, commandNonce, options.commandPayload, sessionKey));
        ...
        const expectedTag = authenticate(sessionKey, RESPONSE_MAGIC, epoch, commandNonce, header.subarray(LENGTH_OFFSET, TAG_OFFSET), payload);
...
function deriveSessionKey(rootKey, epoch) {
...
function encodeAuthenticatedPacket(magic, epoch, nonce, payload, sessionKey) {
...
function authenticate(sessionKey, magic, epoch, nonce, length, payload) {
...
function validateResponseHeader(header, epoch, nonce) {
...
function connectLoopback(socket, port) {
...
function writeAll(socket, payload) {
    return new Promise((resolve, reject) => {
