...
export const INBOUND_PROTOCOL = "lattice-openclaw-inbound" as const;
export const CLIENT_HELLO_PROTOCOL = "lattice-openclaw-client-hello" as const;
...
export function decodeCommandReply(
  payload: Buffer,
  command: LatticeCommand,
  identities: CommandIdentities,
): DecodedCommandReply {
  ...
  try {
    ...
    if (canonicalJson(parsed as CanonicalValue) !== source) {
      throw new LatticeWireError();
    }
  } catch {
    throw new LatticeWireError();
  ...
  if (command.action === "submit") {
    if (
      frame.protocol !== "lattice-openclaw-inbound-reply" ||
  ...
  if (
    frame.protocol !== "lattice-gateway-ipc" ||
...
function gatewayControlRequestDigest(
...
function gatewayControlRequestBody(
  command: Exclude<LatticeCommand, { readonly action: "submit" }>,
): CanonicalValue {
...
function requestBody(command: LatticeCommand): CanonicalValue {
...
function taskTarget(target: TaskTargetArguments): CanonicalValue {
  return {
    binding: {
...
function decodeSubmitBody(value: unknown, taskSpecDigest: string): DecodedCommandReply {
...
function decodeControlBody(value: unknown, command: LatticeCommand): DecodedCommandReply {
  const body = record(value);
  if (body.kind === "denied") {
...
function decodeObservation(
  value: unknown,
  command: Extract<LatticeCommand, { readonly action: "status" }>,
): DecodedCommandReply {
  ...
  if (command.targetKind === "project") {
    ...
    if (
      (observation.next_cursor !== null && typeof observation.next_cursor !== "string") ||
  ...
  if (
    observation.original_command_id !== command.targetCommandId ||
    observation.project_id !== command.projectId ||
...
function validateTaskProjection(value: unknown): {
  ...
  if (
    !validDigest(projection.ledger_head_digest) ||
...
function validateStopTarget(
  value: unknown,
  command: Extract<LatticeCommand, { readonly action: "stop" }>,
): void {
  const target = exactRecord(value, ["attempt_id", "reason", "target"]);
  if (
    target.attempt_id !== command.attemptId ||
...
interface ValidatedBinding extends Record<string, unknown> {
  readonly project_id: string;
  readonly project_snapshot_id: string;
  readonly task_id: string;
  readonly task_revision: string;
  readonly task_spec_digest: string;
}

function validateBinding(value: unknown): ValidatedBinding {
  ...
  for (const key of ["project_id", "project_snapshot_id", "task_id", "task_revision"] as const) {
  ...
  return binding as ValidatedBinding;
}

function requireExactTaskTarget(
  binding: ValidatedBinding,
  ledgerHeadDigest: unknown,
...
function denialCode(value: unknown): string {
...
function unknownCode(value: unknown): string {
