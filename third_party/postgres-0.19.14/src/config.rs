...
impl Config {
    ...
    /// Opens a connection while bounding the complete startup and
    /// authentication exchange, not only each socket connection attempt.
    ///
    /// `Ok(None)` means that the startup deadline elapsed. Dropping the timed
    /// future before the current-thread runtime is destroyed also drops the
    /// in-flight socket, so callers do not retain a detached connection task.
    pub fn connect_with_startup_timeout<T>(
        &self,
        tls: T,
        timeout: Duration,
        ...
        <T::TlsConnect as TlsConnect<Socket>>::Future: Send,
    {
        ...
        let runtime = runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .unwrap(); // Matches the upstream synchronous constructor.

        let connected = runtime
            .block_on(async { tokio::time::timeout(timeout, self.config.connect(tls)).await });
        let (client, connection) = match connected {
            Ok(connected) => connected?,
            Err(_) => return Ok(None),
        };
        let connection = Connection::new(runtime, connection, self.notice_callback.clone());
        Ok(Some(Client::new(connection, client)))
    }
}
