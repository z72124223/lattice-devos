...
/// Computes a lowercase SHA-256 content digest without exposing a hashing
/// dependency or an unvalidated digest constructor to downstream crates.
#[must_use]
#[allow(clippy::many_single_char_names, clippy::unreadable_literal)]
pub fn sha256_content_digest(input: &[u8]) -> ContentDigest {
    const INITIAL: [u32; 8] = [
    ...
    for word in state {
        ...
        write!(&mut hex, "{word:08x}").expect("string write");
    }
    ContentDigest(hex)
}
