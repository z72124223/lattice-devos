...
const CURRENT_GLOBAL_LEDGER_SCHEMA_VERSION: u16 = 4;
...
const LEDGER_PREPARE_V4_SQL: &str = "\
    SELECT stream_found, command_found, retained_request_digest, \
      ...
      FROM control.task_ledger_prepare_v2(\
           $1::smallint, $2::text, $3::bytea, $4::text)";

const LEDGER_HEAD_V4_SQL: &str = "\
    SELECT stream_id, ledger_schema_version, head_contract_version, producer_id, \
      ...
      FROM control.task_ledger_read_head_v2(\
           $1::smallint, $2::text, $3::bytea, $4::text, $5::text)";

const LEDGER_EVENTS_V4_SQL: &str = "\
    SELECT stream_id, event_sequence, event_schema_version, previous_event_digest, \
      ...
      FROM control.task_ledger_read_events_v2($1::smallint, $2::text, $3::bytea)";

const LEDGER_COMMANDS_V4_SQL: &str = "\
    SELECT stream_id, command_id, request_schema_version, request_digest, \
      ...
      FROM control.task_ledger_read_commands_v2($1::smallint, $2::text, $3::bytea)";

const STORE_PREPARE_V4_SQL: &str = "\
...
const STORE_FINALIZE_V4_SQL: &str = "\
...
const STORE_CURRENT_V4_SQL: &str = "\
...
const LEDGER_FINALIZE_V4_SQL: &str = "\
    SELECT control.task_ledger_finalize_v2(\
...
/// Global schema-v4 persistence identity, distinct from Store receipt profile 2.
...
impl PostgresTaskLedger {
    /// Verifies an exact frozen schema-v3 or current schema-v4 runtime surface
...
impl TaskLedgerSqlProfile {
    ...
    const fn ledger_commands_sql(self) -> &'static str {
        match self {
            ...
            Self::V4 => LEDGER_COMMANDS_V4_SQL,
        }
    }
    ...
    const fn ledger_finalize_sql(self) -> &'static str {
        match self {
            ...
            Self::V4 => LEDGER_FINALIZE_V4_SQL,
        }
    }
...
fn global_profile_params<'a, const N: usize>(
    ...
    let mut params =
        Vec::with_capacity(N + usize::from(sql_profile == TaskLedgerSqlProfile::V4) * 2);
    if sql_profile == TaskLedgerSqlProfile::V4 {
...
mod tests {
    ...
    fn runtime_transactions_have_fixed_timeouts_and_queries_append_global_identity() {
        ...
        assert!(LEDGER_HEAD_V4_SQL.contains("task_ledger_read_head_v2("));
        assert!(
            STORE_PREPARE_V4_SQL
                .contains("terminal_receipt_digest, global_schema_version, global_manifest_sha256")
        );
        assert!(
            STORE_CURRENT_V4_SQL
    ...
    #[test]
    fn task_ledger_routes_only_verified_historical_v3_and_current_v4_profiles() {
        assert_eq!(
        ...
        for version in [0, 1, 2, 5, u16::MAX] {
