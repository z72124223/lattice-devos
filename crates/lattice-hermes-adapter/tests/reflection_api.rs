...
fn codex_broker_pins_four_files_and_locks_the_proven_empty_tool_policy() {
    ...
    let remote_control_status = br#"{"emittedAtMs":1750000000000,"method":"remoteControl/status/changed","params":{"environmentId":"env","installationId":"install","serverName":"codex","status":"disabled"}}"#;
    assert_eq!(
...
fn codex_no_marker_plan_and_receipt_require_joint_empty_tool_evidence() {
    ...
    assert!(thread.get("jsonrpc").is_none());
    ...
    assert!(turn.get("jsonrpc").is_none());
    assert_eq!(turn["id"], 2);
...
fn codex_broker_protocol_reconciles_terminal_before_turn_response() {
    ...
    let mut protocol =
        CodexBrokerProtocol::new(codex_home.clone(), cwd.clone(), "gpt-5.6-terra")
...
fn new_codex_protocol_fixture() -> (CodexBrokerProtocol, std::path::PathBuf) {
    ...
    std::fs::create_dir_all(&codex_home).expect("fixture codex home");
    std::fs::create_dir_all(&cwd).expect("fixture cwd");
    let protocol = CodexBrokerProtocol::new(codex_home, cwd, "gpt-5.6-terra")
        .expect("bounded fixture protocol");
