...
function Assert-DeliveryRunEvidence {
    ...
    if ($actualProperties.Count -ne $required.Count -or @($actualProperties | Where-Object { $_ -notin $required }).Count -ne 0) {
        throw 'LATTICE_DELIVERY_RUN_EVIDENCE_NOT_STRICT'
    }
    Assert-ExactJsonEvidenceTypes `
        -Evidence $Evidence `
        -StringFields @($required | Where-Object { $_ -notin @('schema_file_count', 'changed_paths') }) `
        -IntegerFields @('schema_file_count') `
...
function Assert-DurableStatusEvidence {
    ...
    $actualProperties = @($Evidence.PSObject.Properties.Name)
    if ($actualProperties.Count -ne $required.Count -or @($actualProperties | Where-Object { $_ -notin $required }).Count -ne 0) {
        throw 'LATTICE_DELIVERY_STATUS_EVIDENCE_NOT_STRICT'
    }
    Assert-ExactJsonEvidenceTypes `
        -Evidence $Evidence `
        -StringFields @($required | Where-Object { $_ -notin @('schema_file_count', 'changed_paths') }) `
...
function Invoke-DeliveryRunPhase {
    ...
    $repositoryOwnedPaths = @($fixtureRoot, $runtime, $schemaDirectory, $deliveryRoot, $runEvidencePath)
    if ($codexMode -eq 'SCRIPTED_ACCEPTANCE') {
