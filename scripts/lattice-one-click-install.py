...
def write_workspace_hook(source: Path) -> None:
    hook = """\n\n<!-- BEGIN LATTICE MANAGED v1 -->\n## LATTICE 工作區掛勾\n新 Codex 工作開始前先執行 `lattice_runtime_status`，確認 LATTICE Runtime 與三核心狀態；不要繞過 LATTICE 的驗收或自行啟用已退役的 Hermes。\n<!-- END LATTICE MANAGED v1 -->\n"""
    ...
    if begin in existing:
        ...
        path.write_text(before + hook + ("\n" + after if after else "\n"), encoding="utf-8")
    else:
        path.write_text(existing.rstrip() + hook, encoding="utf-8")
...
def run_install(bundle: Path, state: Path, source: Path, wsl: Path, platform_root: Path | None,
    ...
    if result.returncode == 0 and payload.get("status") != "BLOCKED":
        ...
        for action, extra in (("register-project", ["--project-root", str(source), "--project-name", project_name]),
            ...
            if step.returncode: payload["status"] = "BLOCKED"; break
        if payload.get("status") != "BLOCKED" and codex_config:
            step = subprocess.run(common + ["connect", "--codex-config", str(codex_config)], capture_output=True, text=True,
                                  encoding="utf-8", errors="replace", creationflags=subprocess.CREATE_NO_WINDOW if os.name == "nt" else 0)
            steps.append({"action": "connect", "exit_code": step.returncode, "output": step.stdout[-2000:]})
            if step.returncode: payload["status"] = "BLOCKED"
        if payload.get("status") != "BLOCKED": write_workspace_hook(source)
        payload["post_install_steps"] = steps
    return payload
...
def main() -> int:
    ...
    parser.add_argument("--graph-source", type=Path, default=Path.cwd()); parser.add_argument("--wsl", type=Path, default=Path(os.environ.get("SystemRoot", "C:/Windows")) / "System32" / "wsl.exe")
    parser.add_argument("--graphify-platform", type=Path); parser.add_argument("--codex-config", type=Path,
