...
- Stage 1 durable HOME-PREPARE failure receipt commit is `40e1de22ddc2552f58a5f77656955083af383ad1`; expected and live remote SHA matched at `2026-08-11T21:20:13.3474843Z`. This saver-only update is the stage 2 confirmation; after its fresh live `git ls-remote` equality passes, outer saver state is `safe_to_archive=true`. Authoritative receipt remains a pre-provision failure with `safe_to_archive=false`; neither `HOME_PREPARED_ONLY` nor P0 PASS is claimed.
