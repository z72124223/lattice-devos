...
function Assert-NoDuplicateJsonObjectFields {
    ...
    for ($index = 0; $index -lt $Text.Length; $index++) {
        ...
        if ($character -eq '"') {
            ...
            if ($index -ge $Text.Length) { throw 'CONFORMANCE_MANIFEST_JSON_REJECTED' }
            $lookAhead = $index + 1
            ...
            if ($lookAhead -lt $Text.Length -and $Text[$lookAhead] -eq ':') {
                if ($scopes.Count -eq 0) { throw 'CONFORMANCE_MANIFEST_JSON_REJECTED' }
                ...
                if ($name -isnot [string]) { throw 'CONFORMANCE_MANIFEST_JSON_REJECTED' }
                $scope = $scopes[$scopes.Count - 1]
                if (-not $scope.Add([string]$name)) { throw 'CONFORMANCE_MANIFEST_DUPLICATE_FIELD' }
            }
            continue
        }
        if ($character -eq '{') {
    ...
    if ($scopes.Count -ne 0) { throw 'CONFORMANCE_MANIFEST_JSON_REJECTED' }
}
...
try {
    ...
    if (($manifest.registry.type_count -isnot [int] -and $manifest.registry.type_count -isnot [long]) -or
        [long]$manifest.registry.type_count -ne 1 -or
        $manifest.registry.mapped_intent -isnot [string] -or
        [string]$manifest.registry.mapped_intent -cne 'CONTROLLED_CODEX_CANARY') {
        throw 'CONFORMANCE_MANIFEST_REGISTRY_REJECTED'
    }
    $registryTypeCount = 1
    $mappedIntent = 'CONTROLLED_CODEX_CANARY'
    ...
    for ($caseIndex = 0; $caseIndex -lt $cases.Count; $caseIndex++) {
        ...
        if ($expectedResult -ceq 'accepted') {
            ...
            if ([string]$projection.contract_schema -cne 'lattice.task-contract.v1' -or
                [string]$projection.contract_type -cne 'controlled_codex_canary' -or
                ...
                [string]$projection.intent -cne 'CONTROLLED_CODEX_CANARY' -or
                $submitFields.Count -ne 2 -or
                [string]$submitFields[0] -cne 'client_request_id' -or
            ...
            $expectedProjectionText = [ordered]@{
                ...
                mcp_tool = [string]$case.expected.mcp_tool
    ...
    if ($acceptedCount -ne 1 -or $rejectedCount -ne ($caseCount - 1)) { throw 'CONFORMANCE_MANIFEST_OUTCOME_COUNT_REJECTED' }
