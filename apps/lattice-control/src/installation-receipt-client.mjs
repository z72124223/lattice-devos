...
const defaultControlOrigin = "http://127.0.0.1:4317";
...
async function requestJson(fetchImpl, url, options = {}, timeoutMs = 5_000) {
  ...
  if (!response.ok) throw new Error(body.error || `Control returned HTTP ${response.status}`);
...
function resolveProject(projects, { projectId, projectName }) {
