...
test("the resizable desktop can reach compact scrolling without desktop or mobile regressions", async () => {
  ...
  assert.equal(minimumWidth, 360);
  assert.equal(minimumHeight, 320);
  ...
  assert.doesNotMatch(windowCode, /SourceInitialized \+=/u);
  assert.match(windowCode, /protected override void OnSourceInitialized\(EventArgs e\)[\s\S]*?base\.OnSourceInitialized\(e\)[\s\S]*?EnableNativeResizeFrame\(handle\)[\s\S]*?AddHook\(WindowProc\)/u);
  ...
  assert.match(windowCode, /EnableNativeResizeFrame\(handle\)/u);
  assert.match(windowCode, /WsThickFrame/u);
  assert.match(windowCode, /SetWindowLongPtr/u);
  assert.match(windowCode, /Marshal\.SetLastPInvokeError\(0\)[\s\S]*?GetWindowLongPtr/u);
  assert.match(windowCode, /currentStyle == nint\.Zero[\s\S]*?Marshal\.GetLastPInvokeError\(\) != 0[\s\S]*?return/u);
  assert.match(windowCode, /previousStyle == nint\.Zero[\s\S]*?Marshal\.GetLastPInvokeError\(\) != 0[\s\S]*?return/u);
  assert.match(windowCode, /SetWindowPos[\s\S]*SwpFrameChanged/u);
  ...
  assert.match(compactCss, /\.workspace-canvas \{ display:block; min-height:0; overflow:visible; \}/u);
  assert.match(compactCss, /\.core-stage \{ min-height:520px; overflow:visible; \}/u);
  assert.match(compactCss, /\.core-panel,\.panel \{ height:auto; min-height:520px; \}/u);
  assert.match(compactCss, /\.graph-viewport \{ height:500px; min-height:500px; overflow-x:auto; overflow-y:hidden; overscroll-behavior-x:contain; \}/u);
  assert.match(compactCss, /\.transcript \{ padding:13px; overflow:visible; overscroll-behavior-y:auto; \}/u);
  assert.match(compactCss, /\.scroll-body \{ height:auto; overflow:visible; padding:12px; \}/u);
